// The C twin of select_par.bend: the same clustered grounds, the same
// fixed-point
...
//
// This twin is sequential and the Bend side is not, which is the point: the
// shards are independent, so the same 128 leaf checksums come out whatever
// order they are computed in, and `par` folds them in a fixed binary tree that
// does not depend on which lane finished first.
#include <stdint.h>
#include <stdio.h>
...
// select_par.bend's fork tree, in order: a leaf is a shard and a node mixes its
// left subtree's checksum with its right's
static uint32_t par(uint32_t d, uint32_t s) {
  if (d == 0) return shard(s);
  uint32_t w = 1u << (d - 1);
  uint32_t a = par(d - 1, s), b = par(d - 1, s + w);
  return mixc(a, b);
}
