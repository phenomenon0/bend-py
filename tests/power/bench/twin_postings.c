// The C twin of tests/power/bench/postings.bend: the same 65,536-document
// universe, the same 4,096 rounds of 8 builds and 4 operations, the same
// sparse-or-dense decision at the same threshold, and the same fold.
//
// The checksum is over set CONTENTS -- every member id of every answer, in
// order -- and not over the words that happen to hold them. So this file is
// free to lay a set out however it likes and still has to agree with Bend id
// for id. That is the property worth pinning: a checksum over representation
// would make sparse and dense disagree by construction and would have to be
// weakened until it proved nothing.
//
// A build per operand, like the Bend side. There an Array has one owner and an
// operation consumes both operands, so a round cannot reuse a set; here it
// could, and deliberately does not -- reusing them would time a different
// program and flatter this column.
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static uint32_t mix(uint32_t h) { return h ^ (h >> 16); }
static uint32_t hsh(uint32_t s) { return mix((s + 1u) * 2654435761u); }
...
int main(void) {
  uint32_t acc = 0;
  for (uint32_t i = 0; i < 4096u; i++) acc = rnd(i * 8u, acc);
  printf("%u\n", acc);
  return 0;
}
